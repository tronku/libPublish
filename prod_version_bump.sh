...
#if tag is beta tag, remove the last 2 digits
number_of_dots=$(echo $git_latest_tag | grep -o "\." | wc -l)
if [ $number_of_dots == '3' ] ; then
  git_latest_tag=${git_latest_tag:0:7}
fi

git_upcoming_tag="$(echo $git_latest_tag | gawk -F. -v OFS=. 'NF==1{print ++$NF}; NF>1{if(length($NF+1)>length($NF))$(NF-1)++; $NF=sprintf("%0*d", length($NF), ($NF+1)%(10^length($NF))); print}')"
